...
Wisp.getLeagueRank = getLeagueRank;
...
function isLvLBattle(p1, p2, id, status, types, score) {
	...
	if (Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenger && Rooms.global.LvL[leagueId].challenger === targetLeagueid || Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenging && Rooms.global.LvL[leagueId].challenging === targetLeagueid) {
		...
		if (status === 'start') {
			...
			lvlDisplay(room);
			room.add("|raw|The League vs League battle between " + Wisp.nameColor(p1, true) + " and " + Wisp.nameColor(p2, true) + " ended with a tie. They have to have a rematch.");
			room.update();
		}
	}
}
Wisp.isLvLBattle = isLvLBattle;
...
exports.commands = {
	...
	league: {
		...
		accept: function (target, room, user) {
			...
			leaguePM(Wisp.nameColor(user.name, true) + " has accepted their invitation to join the league.", leagueid);
		...
		decline: function (target, room, user) {
			...
			leaguePM(Wisp.nameColor(user.name, true) + " has declined their invitation to join the league.", leagueid);
		...
		leave: function (target, room, user) {
			...
			leaguePM(Wisp.nameColor(user.name, true) + " has left the league.", leagueid);
		...
		registerteam: function (target, room, user) {
			...
			getTeam(pastebin, (err, team) => {
				...
				leaguePM(Wisp.nameColor(user.name, true) + " has registered their team to become a challenger.", toId(league));
		...
		challenge: function (target, room, user, connection) {
			...
			user.prepBattle(Tools.getFormat('ubers').id, 'challenge', connection).then(result => {
				if (result) {
					...
					targetUser.send('|pm|' + user.getIdentity() + '|~|/raw <div class="infobox">' + Wisp.nameColor(user.name, true) + ' is challenging your league.</div>');
					user.send('|pm|' + targetUser.getIdentity() + '|~|/raw <div class="infobox">You are challenging ' + Wisp.nameColor(targetUser.userid, true) + '\'s league.');
					leaguePM(Wisp.nameColor(user.name, true) + " has challenged " + Wisp.nameColor(targetUser.name, true) + ".");
				}
			});
		},
		...
		badge: {
			...
			view: function (target, room, user) {
				...
				if (!this.runBroadcast()) return;
				let badges = Wisp.getBadges(target);
		...
		lvl: {
			...
			challenge: function (target, room, user) {
				...
				let lvlId = Wisp.randomString(10);
		...
		points: {
			give: function (target, room, user) {
				...
				leaguePM(Wisp.nameColor(user.name, true) + " has given your league " + amount + (amount === 1 ? " point." : " points."), league);
			},
			...
			take: function (target, room, user) {
				...
				leaguePM(Wisp.nameColor(user.name, true) + " has taken " + amount + (amount === 1 ? " point " : " points ") + " from your league.", league);
			},
			...
			reset: function (target, room, user) {
				...
				Wisp.messageSeniorStaff("/html " + Wisp.nameColor(user.name, true) + " has reset all league points.");
				Rooms('upperstaff').add("|raw|" + Wisp.nameColor(user.name, true) + " has reset all league points.").update();
